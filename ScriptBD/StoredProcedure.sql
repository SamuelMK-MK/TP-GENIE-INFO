...
create procedure sp_insert_personne
	@id int,@matricule varchar(50),@nom varchar(50),@postnom varchar(50),
...
begin
	if not exists(select * from personne where id=@id)
		...
		(@id,@matricule,@nom,@postnom,@prenom,@sexe)
	else
		update personne set matricule=@matricule, nom=@nom,postnom=@postnom,prenom=@prenom,
